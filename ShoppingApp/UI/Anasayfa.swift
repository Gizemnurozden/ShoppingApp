...
extension Anasayfa: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let kategori = kategorilerListesi[indexPath.row]
        performSegue(withIdentifier: "toShopping", sender: kategori.ad)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "toShopping" {
            if let kategoriAd = sender as? String {
                let gidilecekVC = segue.destination as! Shopping
                gidilecekVC.selectedCategory = kategoriAd
            }
        }
    }
}
